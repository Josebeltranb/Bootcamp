...
export default function Register() {
  ...
  const wordToCapitalize = words => {
    ...
    if (words !== null && words !== undefined && words.length > 0) {
      const toCapitalizeArray = words.split(" ");

      toCapitalizeArray.forEach((word, index) => {
        if (index === 0) {
          toCapitalize = `${word[0].toUpperCase()}${word.slice(1)}`;
        } else {
          toCapitalize = word && word.length > 0 && `${toCapitalize} ${word[0].toUpperCase()}${word.slice(1)}`;
        }
      });
    }
